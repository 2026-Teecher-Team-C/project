...
> **2026-09-21 갱신: 이 제안은 구현·재측정됐다** (PoC 저장소 커밋 `ff59372`). 같은 4분 웹서핑
> 조건에서 139건 → 20건, 그중 순수 오탐은 3건이다. 규칙 3은 A안(`Content-Length`가 있을 때만
> 크기 폴백)을 채택했다 — 정상 트래픽만 보면 B안이 근소하게 유리하지만, 공격자는 자기 서버를
> 통제하므로 `Content-Disposition`을 빼고 제외 목록에 없는 MIME을 주면 B안에서는 그 다운로드가
> **관측되지 않는다.** 못 잡는 비용과 잘못 잡는 비용은 대칭이 아니다. 아래 원문은 제안 당시
> 기록으로 남긴다.
...
> **2026-09-21 갱신: 구현됐다** (PoC 저장소 커밋 `ff59372`). 비다운로드 트래픽의 3초 지연이
> 사라졌고, 부수로 `mime_type` 오염 버그도 닫혔다 — 403 교체 전 `responseheaders` 시점에 원본
> `Content-Type`을 스냅샷하므로 스키마 변경이 필요 없다. 아래 원문은 제안 당시 기록이다.
...
- [x] 6장 규칙을 실제 addon 코드로 구현하고 재측정 (PoC 커밋 `ff59372`) — 139건 → 20건
      (테스트 다운로드 4 + Chrome 자동 업데이트 13 + 순수 오탐 3). 규칙 3은 A안 채택
- [x] 7장 `responseheaders` 스트리밍 구조 구현 (같은 커밋) — 비다운로드 트래픽 지연 해소,
      `mime_type` 오염 버그 동반 해결
- [x] 재측정이 드러낸 fail-close 부작용을 제품 설계 6장에 반영하고, `bypass_domains`에
      `category`(`PINNED` / `SECURITY_UPDATE`)와 와일드카드 금지 제약을 추가 (ERD 문서 2.4)
- [ ] `SECURITY_UPDATE` 바이패스의 정확한 호스트 목록 확정 — 재측정 로그에서 추출 필요
- [ ] 검증 기준 재정의: "이벤트 0건"은 Chrome 자동 업데이트 때문에 달성 불가능한 목표였다.
      "순수 오탐 0건"으로 바꿔야 한다
- [ ] 남은 오탐 3건 — `application/x-protobuf` 제외 목록 누락, `octet-stream`을 fetch하는
      Brotli 사전, `Sec-Fetch` 없는 폴백 경로. 원인이 각각 달라 단일 수정으로 닫히지 않는다
- [x] 8장 항목 2·3 (`file_size` NULL 서술 정정, `mime_type`이 실제 NULL 칸이라는 사실 기록)을
      `2026-09-17-erd-review-and-revision.md`의 "논리 오류" 6번과 DDL 주석에 반영. `file_size`를
      NOT NULL로 두는 안은 검토 항목으로만 남기고 `schema.sql`의 실제 제약은 바꾸지 않음
      (스트리밍 전환 작업이 진행 중이라 확정 단계가 아님) — `schema.sql`은 애초에 두 컬럼 모두
      제약을 바꿀 필요가 없어 수정 없음
- [x] 8장 "피닝된 앱" 관련 와일드카드 주의사항을 제품 설계 8장 "비대응 범위" 표 아래 각주로 반영
      (`프록시-악성코드-탐지-플랫폼.md`, 이 작업 시작 이전에 이미 반영되어 있던 것을 확인함)
- [ ] 8장 항목 1 (`mime_type` 오염 — BLOCKED 건에서 403 응답 헤더로 덮어써짐) — **보류.** 원인이
      PoC 코드의 헤더 읽기 순서 문제이고 스키마 변경 없이 해결 가능해 PoC 저장소 코드로 처리
      중이므로 스키마 문서에는 반영하지 않음
- [ ] 8장 항목 5 (보존·마스킹 주기 재검토) — **보류 사유 해소, 이제 판단 가능.** 재설계 후
      시간당 약 2,000행이 약 300행 수준으로 내려왔고, `SECURITY_UPDATE` 바이패스를 적용하면
      더 내려간다. 확정된 수치가 나오면 erd-review 6장의 마스킹 주기를 재검토할 것
