...
에이전트 → nginx:443 요청 실패/타임아웃
    ...
    └─ ~~로컬 SQLite 캐시로 폴백~~ [초기 단계 미구성]
            ...
            └─ 캐시에 없음 → 정책값(fail-open/fail-close)에 따라 **즉시** 통과 or 차단
               (재시도·서킷브레이커·로컬 캐시 없이, 이미 본 적 있는 파일이든 처음 보는 파일이든
               구분 없이 동일하게 정책값만 적용 — 초기 단계 실제 흐름)
```
...
- fail-open/fail-close 정책값은 관리 콘솔의 정책 편집 기능(제품 설계 문서 3장)을 통해 설정하며,
  이 설계 문서에서 별도 인프라 요소를 추가하지 않는다. (초기 단계에는 관리 콘솔도 없으므로 정책값은
  설정 파일 또는 하드코딩으로 임시 관리)
